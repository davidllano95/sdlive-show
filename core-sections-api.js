...
async function readJsonBody(request) {
  ...
  if (text.length > 180000) throw new Error("Request body is too large");
  return JSON.parse(text);
}

function sectionFromPath(path) {
  const match = path.match(/^\/api\/(?:admin\/)?content\/(about|services|work|international)(?:\/.*)?$/);
  return match?.[1] || null;
}
...
function entryFromRow(section, row) {
  ...
  if (!row) {
    return {
      ...
      draft: cloneCoreSectionDefault(section),
      published: cloneCoreSectionDefault(section)
    };
  }
...
async function saveDraft(env, section, serializedDraft, userEmail) {
  const key = CORE_SECTION_KEYS[section];
  const current = await getRow(env, section);

  if (current) {
    await env.CMS_DB.batch([
      env.CMS_DB.prepare(`UPDATE cms_entries SET draft_json = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).bind(serializedDraft, current.id),
      env.CMS_DB.prepare(`
        INSERT INTO cms_revisions (section, market, route, content_json, revision_type, actor_email)
        VALUES (?, ?, ?, ?, 'save', ?)
      `).bind(key.section, key.market, key.route, serializedDraft, userEmail)
    ]);
  } else {
    const publishedDefault = JSON.stringify(CORE_SECTION_DEFAULTS[section]);
    await env.CMS_DB.batch([
      env.CMS_DB.prepare(`
        INSERT INTO cms_entries (section, market, route, draft_json, published_json, updated_at, published_at)
        VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      `).bind(key.section, key.market, key.route, serializedDraft, publishedDefault),
      env.CMS_DB.prepare(`
        INSERT INTO cms_revisions (section, market, route, content_json, revision_type, actor_email)
        VALUES (?, ?, ?, ?, 'save', ?)
      `).bind(key.section, key.market, key.route, serializedDraft, userEmail)
  ...
  return entryFromRow(section, await getRow(env, section));
}

async function publish(env, section, userEmail) {
  const key = CORE_SECTION_KEYS[section];
  const current = await getRow(env, section);
  ...
  await env.CMS_DB.batch([
    env.CMS_DB.prepare(`
      UPDATE cms_entries
      SET published_json = draft_json, published_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `).bind(current.id),
    env.CMS_DB.prepare(`
      INSERT INTO cms_revisions (section, market, route, content_json, revision_type, actor_email)
      VALUES (?, ?, ?, ?, 'publish', ?)
    `).bind(key.section, key.market, key.route, draftText, userEmail)
  ]);
...
export async function handleCoreSectionsApi(request, env, { verifyAdmin = null } = {}) {
  ...
  if (path === publicPath && request.method === "GET") {
    try {
      return json(publicPayload(section, await getRow(env, section)));
    } catch (error) {
  ...
  if (path === adminPath && request.method === "GET") {
    try {
      return json({ ok: true, entry: entryFromRow(section, await getRow(env, section)) });
    } catch (error) {
      return json({ ok: false, error: `Could not read ${section} content`, detail: String(error?.message || error) }, 500);
    }
  }
  ...
  if (path === adminPath && request.method === "PUT") {
    try {
      ...
      return json({ ok: false, error: `Could not save ${section} draft`, detail: String(error?.message || error) }, 400);
    }
  }

  if (path === `${adminPath}/publish` && request.method === "POST") {
    try {
      const entry = await publish(env, section, user.email);
      return json({ ok: true, message: `${section} published to D1`, entry });
    } catch (error) {
      return json({ ok: false, error: `Could not publish ${section}`, detail: String(error?.message || error) }, 400);
    }
  }

  if (path === `${adminPath}/revisions` && request.method === "GET") {
    try {
      return json({ ok: true, revisions: await revisions(env, section) });
    } catch {
      return json({ ok: false, error: `Could not read ${section} revision history` }, 500);
    }
  }
