...
test("verifies Meta sha256 webhook signatures", async () => {
  ...
  assert.equal(
    await verifyMetaWebhookSignature(rawBody, `sha256=${digest}`, secret, { cryptoImpl: webcrypto }),
    true
  );
  assert.equal(
    await verifyMetaWebhookSignature(rawBody + "x", `sha256=${digest}`, secret, { cryptoImpl: webcrypto }),
    false
  );
});

test("extracts only message payloads for the exact configured phone number id", () => {
...
test("GET webhook verification returns Meta challenge even while runtime execution is disabled", async () => {
  const request = new Request(
    "https://sdlive.show/api/webhooks/whatsapp?hub.mode=subscribe&hub.verify_token=verify-me&hub.challenge=abc123"
  );
  const response = await handleWhatsAppOwnerWebhook(request, env({ WHATSAPP_OWNER_CONTROL_ENABLED: "false" }));
...
test("rejects invalid Meta signature before kill switch or command execution", async () => {
  let executed = false;
  const response = await handleWhatsAppOwnerWebhook(
    webhookRequest(payload()),
    env({ WHATSAPP_OWNER_CONTROL_ENABLED: "false" }),
    {
...
test("signed events are acknowledged without side effects while kill switch is off", async () => {
  let executed = false;
  let claimed = false;
  const store = {
    async claim() { claimed = true; return { isNew: true, row: null }; }
  };
  const response = await handleWhatsAppOwnerWebhook(
    webhookRequest(payload()),
    env({ WHATSAPP_OWNER_CONTROL_ENABLED: "false" }),
    {
      ...
      executeCommand: async () => { executed = true; return { reply: "no" }; },
      store,
      sendText: async () => {}
    }
  );
...
test("signed owner message aimed at a different phone number id is ignored", async () => {
  let executed = false;
  const response = await handleWhatsAppOwnerWebhook(
    webhookRequest(payload({ phoneNumberId: "987654321" })),
    env(),
    {
      verifySignature: async () => true,
      executeCommand: async () => { executed = true; return { reply: "no" }; },
...
test("an in-flight duplicate is retryable and never executes Availability twice", async () => {
  const store = fakeStore();
...
test("Meta reply failures never expose provider response bodies", async () => {
  await assert.rejects(
    () => sendWhatsAppText(env(), "573001112233", "status", {
      fetchImpl: async () => new Response("SUPER_SECRET_PROVIDER_DETAIL", { status: 500 })
    }),
    (error) => {
      assert.match(String(error.message), /whatsapp_reply_failed_500/);
      assert.doesNotMatch(String(error.message), /SUPER_SECRET_PROVIDER_DETAIL/);
      return true;
    }
  );
});
