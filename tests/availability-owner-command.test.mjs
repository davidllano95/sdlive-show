...
test('until commands calculate a bounded timer in the active Availability timezone', () => {
  const result = parseAvailabilityOwnerCommand('away until 23:00', {
    now,
    timeZone: 'America/Bogota'
  });
  ...
  const madrid = parseAvailabilityOwnerCommand('ausente hasta 23:00', {
    now,
...
test('back clears only the temporary override and status is read-only', () => {
  ...
  assert.deepEqual(parseAvailabilityOwnerCommand('estado', { now }), {
    ok: true,
    type: 'status'
  });
});
