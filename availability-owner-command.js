const STATUS_ALIASES = new Map([
  ...
  ["limited", "limited"], ["limitado", "limited"], ["limitada", "limited"],
  ["away", "away"], ["ausente", "away"]
]);
...
function parseRelativeDuration(value) {
  ...
  if (match) return validateDuration(Number(match[1]) * 60 + Number(match[2] || 0));
  return { ok: false, error: "invalid_duration" };
}
...
function durationUntilClock(value, now, timeZone) {
  ...
  const targetDate = targetMinuteOfDay <= currentMinuteOfDay ? addCalendarDays(current, 1) : current;
  const target = localDateTimeToUtc({ ...targetDate, hour, minute }, timeZone);
  return validateDuration(Math.ceil((target.getTime() - now.getTime()) / 60000));
}

export function parseAvailabilityOwnerCommand(text, { now = new Date(), timeZone = "America/Bogota" } = {}) {
  ...
  if (["status", "estado"].includes(command)) return { ok: true, type: "status" };
  if (["back", "auto", "volver", "regresar"].includes(command)) {
    return { ok: true, type: "override", payload: { action: "override", mode: "auto" } };
  }

  const firstSpace = command.indexOf(" ");
  if (firstSpace < 1) return { ok: false, error: "missing_duration" };
  const mode = STATUS_ALIASES.get(command.slice(0, firstSpace));
  ...
  return {
    ok: true,
    type: "override",
    payload: { action: "override", mode, durationMinutes: duration.minutes },
    timeZone: zone
  };
}
